...
class WorkflowEngine:
    ...
    def create_llm_for_workflow(self, workflow_config: Dict) -> LLM:
        """Create an LLM instance based on workflow configuration"""
        # Get model from workflow config, fallback to default
        model = workflow_config.get("model", self.default_llm_config["model"])
        ...
        return LLM(
            ...
            model=model,
            base_url=self.default_llm_config["base_url"]
        )
    ...
    def create_crew(self, workflow_name: str, inputs: Dict[str, Any],
        ...
        try:
            ...
            # Create LLM instance for this workflow
            workflow_llm = self.create_llm_for_workflow(metadata)
