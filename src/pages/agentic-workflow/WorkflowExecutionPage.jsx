...
const WorkflowExecutionPage = () => {
    ...
    useEffect(() => {
        const fetchWorkflow = async () => {
            try {
                const response = await fetch(`http://localhost:9201/workflows/${workflowId}`);
    ...
    const handleExecute = async (mode) => {
        ...
        try {
            const endpoint = mode === 'test'
                ? `http://localhost:9201/workflows/execute`  // Updated endpoint
                : `http://localhost:9201/workflows/train`;
